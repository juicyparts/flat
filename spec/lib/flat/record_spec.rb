...
describe Record do
  ...
  describe Record::Definition do
    ...
    it 'returns all attributes in as_hash' do
      flat_file.add_field :field, width: 25
      record = Record::Definition.new flat_file, { field: 'Field' }
      record.field = record.field.upcase
      expect(record.attributes).to eq(record.as_hash)
    end

    it 'returns all attributes in as_json' do
      flat_file.add_field :field, width: 25
      record = Record::Definition.new flat_file, { field: 'Field' }
      record.field = record.field.upcase
      data = JSON.parse(record.as_json)
      expect(data['field']).to eq(record.field)
    end
  end
end
